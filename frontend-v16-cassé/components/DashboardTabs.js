...
export function CategoriesTab({ result }) {
  ...
  return (
    <div className="space-y-6">
      {/* === TOP 5 & FLOP 5 (vue rapide pour décideurs) === */}
      {data.length >= 2 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Top 5 — Les plus vendus */}
          <div className="bg-white rounded-lg border-2 border-emerald-200 p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <h3 className="text-base font-bold text-gray-900">
                Top {Math.min(5, top5.length)} — Articles les plus vendus
              </h3>
            </div>
            <p className="text-xs text-gray-500 mb-4">
              Concentrez les efforts marketing et stock sur ces catégories
            </p>
          ...
          {/* Flop 5 — Les moins vendus */}
          {flop5.length > 0 && (
            <div className="bg-white rounded-lg border-2 border-red-200 p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xl">⚠️</span>
                <h3 className="text-base font-bold text-gray-900">
                  Flop {flop5.length} — Articles les moins vendus
                </h3>
              </div>
              <p className="text-xs text-gray-500 mb-4">
                À analyser : faut-il les promouvoir, les réviser, ou les retirer ?
              </p>
              <div className="space-y-2">
                {flop5.map((d, i) => {
                  const pct = total ? (d.value / total) * 100 : 0;
