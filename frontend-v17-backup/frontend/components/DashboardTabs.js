    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 rounded-r-lg p-4">
        ...
        <p className="text-sm text-blue-800 mt-1">
          Visualisation des ventes historiques, calcul des statistiques descriptives,
          identification de la tendance, et premier aperçu des prévisions futures.
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-emerald-50 border-l-4 border-emerald-500 rounded-r-lg p-4">
        <h2 className="text-base font-semibold text-emerald-900">
          Étape 3 : Test du modèle et prévisions futures
        </h2>
        <p className="text-sm text-emerald-800 mt-1">
          Comparaison ligne par ligne entre les valeurs réelles (y_test) et les prévisions de chaque
          modèle (y_pred) sur la période de test. La précision est calculée par période et globalement.
          Les prévisions futures sont ensuite produites par l'ensemble pondéré des modèles fiables.
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 rounded-r-lg p-4">
        ...
        <p className="text-sm text-amber-800 mt-1">
          Identification statistique des valeurs aberrantes par méthode Z-score (écart &gt; 2σ par
          rapport à la moyenne). Une anomalie peut indiquer un événement exceptionnel à investiguer.
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-rose-50 border-l-4 border-rose-500 rounded-r-lg p-4">
        <h2 className="text-base font-semibold text-rose-900">
          Étape 5 : Articles & Catégories — Top et Flop
        </h2>
        <p className="text-sm text-rose-800 mt-1">
          Identification des articles les plus demandés (Top — à prioriser en stock et marketing)
          et les moins demandés (Flop — à analyser ou réviser). Ventilation complète par catégorie.
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-purple-50 border-l-4 border-purple-500 rounded-r-lg p-4">
        <h2 className="text-base font-semibold text-purple-900">
          Étape 2 : Entraînement et test des modèles AI
        </h2>
        <p className="text-sm text-purple-800 mt-1">
          Quatre modèles de Machine Learning sont entraînés en parallèle sur 80% des données (train),
          puis testés sur les 20% restants jamais vus (test). La précision est calculée comme
          100% − MAPE. Seuil de fiabilité : 70%.
        </p>
      </div>

      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-base font-semibold text-gray-900">
          Prévisions par modèle — vue séparée
        </h3>
        <p className="text-xs text-gray-500 mt-1 mb-5">
          Chaque modèle produit ses propres prévisions. Affichées séparément pour permettre la comparaison.
        </p>

        {/* Grille 2x2 : un mini-graphique par modèle */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {result.models.map((m) => {
            const modelData = result.forecast_labels.map((label, i) => ({
              date: fmtDateFR(label),
              value: m.forecast[i],
            }));
            const v = m.validation || {};
            const isReliable = v.is_reliable;
            const accuracy = v.accuracy_pct;
            return (
              <div key={m.key} className={`rounded-lg border-2 p-4 ${isReliable ? "border-emerald-200 bg-emerald-50/30" : "border-red-200 bg-red-50/30"}`}>
                {/* En-tête du mini-graphique */}
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h4 className="font-semibold text-gray-900 text-sm">{m.name}</h4>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {{
                        linear: "Régression linéaire (tendance long terme)",
                        polynomial: "Régression polynomiale (degré 2)",
                        random_forest: "Random Forest (100 arbres)",
                        arima: "ARIMA(1,1,1) — séries temporelles",
                      }[m.key] || m.name}
                    </p>
                  </div>
                  <div className="text-right">
                    <div className={`text-xs font-medium px-2 py-0.5 rounded ${isReliable ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
                      {isReliable ? "✓ Accepté" : "✗ Rejeté"}
                    </div>
                    {accuracy != null && (
                      <div className={`text-sm font-bold mt-1 ${isReliable ? "text-emerald-700" : "text-red-700"}`}>
                        {accuracy}%
                      </div>
                    )}
                  </div>
                ...
                <div className="h-44">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={modelData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} vertical={false} />
                      <XAxis
                        dataKey="date"
                        tick={{ fontSize: 9, fill: COLORS.subtle }}
                        interval="preserveStartEnd"
                      />
                      <YAxis tickFormatter={fmtK} tick={{ fontSize: 9, fill: COLORS.subtle }} width={40} />
                      <Tooltip formatter={(v) => fmtNum(v)} contentStyle={{ fontSize: 11, borderRadius: 6 }} />
                      <Bar dataKey="value" fill={MODEL_COLORS[m.key]} radius={[3, 3, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
      ...
      {/* Graphique de synthèse final : ensemble pondéré uniquement */}
      <div className="bg-white rounded-lg border-2 border-blue-200 p-6">
        <h3 className="text-base font-semibold text-gray-900">
          Synthèse — Prévision finale (Ensemble pondéré)
        </h3>
        <p className="text-xs text-gray-500 mt-1 mb-4">
          Cette courbe est la <strong>moyenne pondérée des modèles fiables</strong> uniquement (précision ≥ 70%).
          C'est la prévision officielle utilisée pour la prise de décision.
        </p>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart
              data={result.forecast_labels.map((label, i) => ({
                date: fmtDateFR(label),
                ensemble: result.ensemble.forecast[i],
              }))}
              margin={{ top: 10, right: 20, left: 0, bottom: 0 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} vertical={false} />
              ...
              <YAxis tickFormatter={fmtK} tick={{ fontSize: 11, fill: COLORS.subtle }} />
              <Tooltip formatter={(v) => [fmtNum(v), "Prévision finale"]} contentStyle={{ fontSize: 12, borderRadius: 6 }} />
              <Bar dataKey="ensemble" name="Prévision finale" fill={MODEL_COLORS.ensemble} radius={[3, 3, 0, 0]} />
              <Line type="monotone" dataKey="ensemble" stroke="#0e7490" strokeWidth={2.5} dot={{ r: 4 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </div>
