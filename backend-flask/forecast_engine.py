...
GRANULARITY_RULES = {"daily": "D", "weekly": "W", "monthly": "MS", "yearly": "YS"}
...
def build_ensemble(models, horizon, validation=None):
    ...
    if validation and "models" in validation:
        ...
        if reliable_models:
            ...
            reliable_weights_normalized = [w / total for w in reliable_weights]

            ensemble_forecast = []
            for h in range(horizon):
                val = sum(reliable_models[i]["forecast"][h] * reliable_weights_normalized[i]
                          for i in range(len(reliable_models)))
                ensemble_forecast.append(round(float(val), 2))

            # Accuracy de l'ensemble = moyenne pondérée des accuracies des modèles fiables
            ensemble_accuracy = 0.0
            for i, m in enumerate(reliable_models):
                acc = validation["models"][m["key"]].get("accuracy_pct", 0)
                ensemble_accuracy += acc * reliable_weights_normalized[i]
            ...
            return {
                ...
                "weights": {reliable_models[i]["key"]: round(reliable_weights_normalized[i], 3)
                            for i in range(len(reliable_models))},
...
def run_full_forecast(df, granularity="monthly", horizon=6, category_column=None):
    ...
    # Générer de vraies dates futures (ex: "2019-01-01", "2019-02-01") à partir
    # de la dernière période du dataset + la granularité.
    last_date = pd.to_datetime(series[-1]["period"])
    ...
    forecast_labels = [
        (last_date + offset * (i + 1)).strftime("%Y-%m-%d")
